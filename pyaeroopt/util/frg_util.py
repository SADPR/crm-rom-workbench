...
def write_top_scaling(fname, nodes, elems,scaling):
    ...
    for nodeset in nodes:
        f.write(nodeset[0]+' '+nodeset[1]+'\n')
        for k, nnum in enumerate(nodeset[2]):
            f.write(str(nnum)+'  '+str(scaling*nodeset[3][k,0])+' '+
                             str(scaling*nodeset[3][k,1])+' '+str(scaling*nodeset[3][k,2])+'\n')
...
def sower_fluid_split(file2split, msh, con, out, nclust,from_ascii=True, log=None,
                      make_call=True, sower=None):
    ...
    # Build execution string, execute
    exec_str = ("{0:s} -fluid -split -con {1:s} -cluster {2:d} -mesh {3:s} "+
                "-result {4:s}").format(sower, con, nclust, msh, file2split)
...
def sower_fluid_merge(res_file, msh, con, out, name, from_bin=False, log=None,
    ...
    exec_str = "{0:s} -name {1:s} -out {2:s} -width 16 -precision 16".format(exec_str, name, out)
...
def split_nodeset(fname, npartition):
    ...
    for k in range(npartition):
        ...
        if k == 0:
            write_nodeset(fname_partk, X[:int(stride), :] )
        elif k == npartition-1:
            write_nodeset(fname_partk, X[int(k*stride):, :])
        else:
            write_nodeset(fname_partk, X[int(k*stride) :int((k+1)*stride), :])
