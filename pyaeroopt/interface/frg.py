...
class Frg(CodeInterface):
    ...
    def __init__(self, **kwargs):
        ...
        self.matcher   = os.path.expandvars('$MATCHER')
    ...
    def run_matcher(self, log='match.log', make_call=True):
        self.match = run_matcher(self.top, self.feminp, self.geom_pre, log,
                                 make_call, self.matcher)

    def sower_fluid_top(self, cpus, nclust, log='sower.top.log',
                        make_call=True):
        try:
            sower_fluid_top(self.top, self.dec, cpus, nclust, self.geom_pre,
                            log, make_call, self.sower, self.match)
        except AttributeError:  # executes if matcher hasn't been run
            sower_fluid_top(self.top, self.dec, cpus, nclust, self.geom_pre,
                            log, make_call, self.sower, None)
    ...
    def sower_fluid_split(self, file2split, out, nclust, from_ascii=True,
                          log='sower.split.log', make_call=True):
        sower_fluid_split(file2split, self.msh, self.con, out, nclust, from_ascii, log,
                          make_call, self.sower)
